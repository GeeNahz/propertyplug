...
const handleRoute = (v:string) => {
  ...
  if(v.includes('reach out on whatsApp')){
    router.push("https://api.whatsapp.com/send?phone=2348104268727&text=Hi,+you+have+reached+property+plug,+how+may+we+help+you")
  }
  return
} 
  return (
    <Modal open={modal} onOk={handleOk} onCancel={handleCancel}>
      <div className="">
        ...
        <div className="flex flex-col mt-12 gap-3">
            {['use calendly', 'schedule office meetup', 'setup a zoom meeting', 'reach out on whatsApp', 'send us a mail', 'call us'].map((v, i) => {
              return(
                <button key={i} onClick={() => handleRoute(v)} className="bg-[#FCFCFC] text-[#0B2831] py-3 rounded-2xl border border-transparent hover:border-black capitalize font-medium">{v}</button>
              )
            })}
        </div>
      </div>
